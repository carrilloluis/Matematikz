...
\noindent Donde :
\begin{eqnarray*}
n \in \mathbb{Z}^{+} &:& \qquad\text{Es el grado del polinomio}\\
a_n &:& \qquad\text{Coeficiente principal del polinomio}\\
a_0 &:& \qquad\text{Término independiente del polinomio} \\
a_0, a_{n-2}, a_{n-1}, a_{n} \in \mathbb{R} &:& \qquad\text{Coeficientes}
\end{eqnarray*}

\noindent Observaciones :
\begin{eqnarray*}
P(x) = 0 : \text{Se llama polinomio nulo.}
\end{eqnarray*}
...
\paragraph*{Clases de grados}
Grado relativo de un monomio, es el exponente de la variable indicada. En el monomio $P(x, y, z) = \sqrt[4]{3}x^5y^9z^{12}$.
\begin{eqnarray*}
GR(x) &=& 5 \\ 
GR(y) &=& 9 \\ 
GR(z) &=& 12 
